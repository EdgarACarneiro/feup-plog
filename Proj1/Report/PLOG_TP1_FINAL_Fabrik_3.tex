...
Nesta secção apresentamos e explicamos a nossa implementação mais aprofundadamente, como a representação visual do tabuleiro, a função de avaliação de um tabuleiro, a verificação de cumprimento das regras do jogo e a determinação do final do jogo.
...
A interface do utilizador foi feita de forma a ser resiliente a \textit{Input} inválido e a permitir que o jogo nunca acaba abruptamente. É visualmente agradável e simples, melhorando a experiência de jogo para o utilizador. As funções usadas na interface encontram-se maioritariamente definidas nos ficheiros \textbf{input.pl}, \textbf{display.pl} e \textbf{menus.pl}.
